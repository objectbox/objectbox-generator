...
OBX_model* create_obx_model() {
	...
	do {
		...
		if (!obx_model_property(model, "{{$property.Name}}", {{$property.Type}}, {{$property.Id.GetId}}, {{$property.Id.GetUid}})) break;
		{{with $property.Flags}}if (!obx_model_property_flags(model, {{.}})) break;
		{{end -}}
		{{end -}}
